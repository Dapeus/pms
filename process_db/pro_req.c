...
char *deal(char *keyword, char *payload) {	
	...
	if (strcspn(ptr,"\r")+1 == strcspn(ptr,"\n")) {
		...
		// if(strcmp(keyword,"URI") == 0) {
		// 	char *temp= strstr(res,"/");
		// 	// char *ret = strstr(temp," ");
		// 	// for(int i = 0; i < 8; i++) {
		// 	// 	*ret = '\0';
		// 	// 	ret++;
		// 	// }
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	if (size_payload > 0) {
		...
		if(host != NULL){
			...
			printf("(4)Src port: %d\n", ntohs(tcp->th_sport));
			printf("(5)Dst port: %d\n", ntohs(tcp->th_dport));
			printf("   Payload (%d bytes):\n", size_payload+SIZE_ETHERNET + size_ip + size_tcp);
			...
			char *uri = deal("URI",payload);
			if(uri != NULL){
				printf("(7)URI: %s\n",uri);
			}

			char *ua = deal("User-Agent: ",payload);
			if(ua != NULL){
				printf("(8)User-Agent: %s\n",ua);
			}

			char *cookie = deal("Cookie: ", payload);
			if(cookie != NULL){
				printf("(9)Cookie: %s\n\n",cookie);
			}
			char cont1[2048];
			sprintf(cont, "insert into request values(%d,'%s'",count, inet_ntoa(ip->ip_src));
			sprintf(cont1, ",'%s',%d,%d,'%s','%s','%s','%s',%d,'%s')",inet_ntoa(ip->ip_dst), ntohs(tcp->th_sport), ntohs(tcp->th_dport), host, uri, ua, cookie, size_frame, "");
			strcat(cont,cont1);
			// printf("%s\n",cont);
			// free(cookie);
			// free(ua);
...
int main(int argc, char **argv)
{
	...
	mysql_query(conn,"truncate request");
	...
	const char *fname = "data/traffic.pcap"; 
